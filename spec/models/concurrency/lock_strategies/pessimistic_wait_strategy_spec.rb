...
describe Concurrency::LockStrategies::PessimisticWait do
  before do
    ...
    @current_strategy = Concurrency::LockStrategies::PessimisticWait
  end
  ...
  describe 'concurrency' do
    describe 'changing cookies' do
      ...
      it 'should be thread safe and execute all calls in sequence' do
        expect { several_threads_test(:add_cookies) }.to change_model(@bucket, :cookies).by(6)
      end

      it 'should be process safe and execute all calls in sequence' do
        expect { several_processes_test(:add_cookies) }.to change_model(@bucket, :cookies).by(6)
      end
    end
    ...
    describe 'transfer cookies' do
      ...
      it 'should be synchronized while changing to target' do
        thread = run_in_thread { add_cookies(2, @target) }
        expect { transfer_cookies }.to change_model(@bucket, :cookies).by(2)
        thread.join
      end

      it 'should be synchronized while changing to bucket' do
        thread = run_in_thread { add_cookies(2, @bucket) }
        expect { transfer_cookies }.to change_model(@bucket, :cookies).by(4)
        thread.join
      end

      it 'should be thread safe and add cookies from  all calls in sequence' do
        expect { several_threads_test(:transfer_cookies) }.to change_model(@bucket, :cookies).by(6)
      end

      it 'should be thread safe and remove cookies from  all calls in sequence' do
      ...
      it 'should be process safe and add cookies from  all calls in sequence' do
        expect { several_processes_test(:transfer_cookies) }.to change_model(@bucket, :cookies).by(6)
      end

      it 'should be process safe and remove cookies from  all calls in sequence' do
